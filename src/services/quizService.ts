...
export type QuizDTO = {
    ...
    questions: Array<{
        ...
        order: number;
        correctOptionId: string; // now exposed for immediate feedback
        options: Array<{ id: string; labelText?: string; imageUrl?: string }>;
    }>;
};

export const quizService = {
    // Now returns an ARRAY of quizzes for the lesson
    getByLesson: async (lessonId: string) => {
        const res = await fetch(join(API_BASE, `/quizzes/by-lesson/${lessonId}`), {
            headers: auth(),
        });
    ...
    create: async (payload: any) => {
        const res = await fetch(join(API_BASE, `/quizzes`), {
            method: "POST",
            headers: { "Content-Type": "application/json", ...auth() },
    ...
    update: async (id: string, payload: any) => {
        const res = await fetch(join(API_BASE, `/quizzes/${id}`), {
            method: "PUT",
            headers: { "Content-Type": "application/json", ...auth() },
            body: JSON.stringify(payload),
        });
