...
export const progressService = {
  async getUserProgress(userId: string) {
    const res = await fetch(`${API_BASE.replace(/\/+$/,'')}/progress/user/${userId}`, {
      headers: { ...authHeaders() }
    });
  ...
  async ping(lessonId: string, positionSec: number, durationSec: number, completed = false) {
    const res = await fetch(`${API_BASE.replace(/\/+$/,'')}/progress/ping`, {
      method: "POST",
